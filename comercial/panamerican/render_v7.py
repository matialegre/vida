# -*- coding: utf-8 -*-
"""Render + verificacion de los DOS PDF de reefers Cerro Moro (v9.0, 8-sep-2026).

   Salidas (pisan las de la v5.2):
     PRESUPUESTO_CERRO_MORO.pdf          cliente, 2 paginas (3 modulos dobles: 1 ext + 2 int)
     PRESUPUESTO_CERRO_MORO_INTERNO.pdf  interno, marca de agua INTERNO - NO ENVIAR en todas

   Cambia respecto de render_v6.py: importa los generadores v7 y la guarda de fugas suma
   TODO lo de la configuracion vieja (5 modulos, cinco, simples, USD 260, 5.000, 10.400,
   1.550, 2.400, 700, BNA 1.535, 4-sep) para que no se cuele ninguna cuenta anterior.
   ...
   Uso: python render_v7.py
"""
import os, io, re
from playwright.sync_api import sync_playwright
import fitz
import armar_cliente_v7 as ARM
import armar_interno_v7 as INT
...
PROHIBIDAS_CLIENTE = [
    ...
    "850",
    # v9.0: TODO lo de la configuracion de 5 modulos (v8.0) y sus cuentas
    "5 módulos", "4 módulos", "cinco", "Cinco", "simples", "simple", "cuatro m",
    "USD 260", "260", "5.000", "10.400", "11.000", "17.000", "15.800", "1.550", "2.400",
    "700", "7.675.000", "767.500", "15.964.000", "1.535", "4-sep", "5 puntos",
    "los dos reefers de la intemperie lo hace", "uno de los dos reefers de afuera",
    "entre reefers",
    # reglas fijas de Matias
    "Ing.", "en funcionamiento", "instalado en",
]

CLAVES = {
    "cli": ["Qué módulo va en cada reefer", "estanco IP65", "por cada par",
            "No incluye cable ni tendido", "El sexto reefer", "sin costo de equipo",
            "25 metros de cable", "sigue midiendo", "un solo cable de 3 hilos", "3 °C",
            "comparte módulo siga alarmando", "Los 3 módulos montados",
            "a las 2 semanas de iniciado", "a las 15 semanas", "Se acepta con",
            "950", "900", "1.800", "400", "1.650", "4.800", "500 / mes", "50 %",
            "Total a 12 meses", "10.800", "10.200", "16.800", "15.600",
            "3 sondas", "lo hace el cliente", "sus dos", "8-sep-2026", "1.530",
            "PROP-CM-2026-09-08", "8 de septiembre de 2026"],
    "int": ["WhatsApp para Andr", "LA CUENTA DEL CA", "439.700", "286",
            "20 pendientes", "Lo que quedó abierto", "semana 0", "500/mes",
            "10.200", "4.800", "sin costo de equipo", "rehecho como lo armaste vos",
            "Nada de esta tabla se adelanta", "2.º bus", "3 °C"],
}
...
def medir(pdf, nombre, tag, paginas_esperadas):
    ...
    if tag == "cli":
        ...
        def fuga(w):
            # cifras: solo como numero entero, no como parte de otro ("765.000" no es "765" ni "5.000")
            if w[0].isdigit() and w[-1].isdigit():
                return re.search(r"(?<![\d.])" + re.escape(w) + r"(?![\d.])", texto) is not None
            return w in texto
        fugas = [w for w in PROHIBIDAS_CLIENTE if fuga(w)]
        print("  ", "SIN FUGAS" if not fugas else "!! FUGA: %s" % fugas)
        if fugas: ok = False
        for mala in ["19-sep", "10-oct", "14-nov", "28-nov", "19-dic", "5-sep", "26-30", "4-sep"]:
            if mala in texto:
