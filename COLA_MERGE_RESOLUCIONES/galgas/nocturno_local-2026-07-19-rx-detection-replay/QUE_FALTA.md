...
2. **Validación con GALGA FÍSICA + INA333** — todo corre con `DEV_SIMULATE_ADC`. Banco real, comparar contra `data/field_captures/` (ground truth, READ-ONLY). Revalidar umbral v_pp>40mV con señal real. [@muestreador]
   ...
   - 🔀 **EN BRANCH** `nocturno/local-2026-07-19-rx-detection-replay` (pendiente de merge): **primer modelo offline de la alerta DIFERENCIAL del RX** (`esp_rx_receptor.ino::updateAlert` caso 3: `dv=|vA-vB|>=TH_V` OR `ratio<=TH_R`, sostenido `HOLD_SEC`, con clasificación cadena-rota) + **replay contra el ground-truth `in_alert` de las capturas reales**. `tools/rx_detection_replay.py` (stdlib, READ-ONLY) + `tools/test_rx_detection_replay.py` (**17 tests OK**) + `docs/rx-detection-replay.md`. Complementa vpp-audit (07-11) y alert-hold (07-16), que modelaban el auto-disparo del EMISOR, no la decisión del RX. **Resultado:** sensibilidad ~100 %, clasificación de mensaje 99.9-100 %, 0 falsas alarmas en reposo. **Hallazgo para @muestreador (drift cuantificado, NO corregido):** el RX de producción actual sería ~1-4 % más "trigger-happy" que el algoritmo de campo legacy (feb-2026) sobre la misma señal (FP sistemáticos en bordes de evento: +118 en el ensayo principal). Ver `MATI-HQ/diario/nocturno-local-2026-07-19.md`.
