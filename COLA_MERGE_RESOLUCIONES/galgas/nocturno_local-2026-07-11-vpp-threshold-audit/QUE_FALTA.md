...
2. **Validación con GALGA FÍSICA + INA333** — todo corre con `DEV_SIMULATE_ADC`. Banco real, comparar contra `data/field_captures/` (ground truth, READ-ONLY). Revalidar umbral v_pp>40mV con señal real. [@muestreador]
   ...
   - 🔀 **EN BRANCH** `nocturno/local-2026-07-11-vpp-threshold-audit` (pendiente de merge): **mitad de software** del #2 hecha. `tools/vpp_threshold_audit.py` (stdlib, READ-ONLY sobre las capturas) reconstruye el mismo `v_pp = max-min` del firmware desde los buffers crudos `bufA`/`bufB` y caracteriza su distribución por archivo/canal + barrido de umbral. `tools/test_vpp_threshold_audit.py` = 29 checks, **OK**. **Resultado sobre las 12 capturas reales:** en reposo el 40 mV da **0.00 %** de auto-disparos (p99≤12.6 mV, margen >3×); los eventos de rotura llegan a **1555 mV** (detecta con margen 38×). 🔎 **Hallazgo para @muestreador:** el **canal B** se auto-dispararía ~2 % por **vibración de motor** (max 49.4 mV) mientras A queda en 0.00 % → decidir en banco subir B a ~50 mV o revisar su front-end. + anomalía en `132330.csv` (pico 1557 mV rotulado "motor estable"). Análisis completo en `docs/vpp-threshold-audit.md`. **Falta (banco/hardware):** revalidar con galga física + INA333 y fijar umbral final. Ver `MATI-HQ/diario/nocturno-local-2026-07-11.md`.
