...
2. **Validación con GALGA FÍSICA + INA333** — todo corre con `DEV_SIMULATE_ADC`. Banco real, comparar contra `data/field_captures/` (ground truth, READ-ONLY). Revalidar umbral v_pp>40mV con señal real. [@muestreador]
    ...
    - 🔀 **EN BRANCH** `nocturno/local-2026-07-29-vpp-field-characterization` (pendiente de merge): **caracterización offline del v_pp real de campo** contra la ventana EXACTA del firmware (500 muestras = 1 s, `BURST_SAMPLES_NORMAL`). `tools/analyze_vpp_field.py` (stdlib, read-only sobre las capturas) + `tools/test_analyze_vpp_field.py` (9 tests OK) + `docs/vpp-field-characterization.md`. **Dos hallazgos con evidencia:** (1) el v_pp real es **≤14 mV mediana en toda condición normal** (reposo/motor/ensayo) → el ALERT de 40 mV tiene margen enorme y el fix del SCADA del 07-28-b (MONITOR 35 mV) queda **validado, hasta conservador** (el "30 mV" del `config.h:108` era pesimista); (2) **para @firmware** — el self-trigger v3 (`v_pp>40mV`, 1 burst, SIN hold) convierte **dropouts de 0.0 V** en ALERTAS espurias (3.0 % de ventanas en `132330` motor ON; 0 % con guard). El legacy tenía `HOLD_SEC=1.5s`; el informe §9.2 pide 2–3 s. Fix candidato (guard de outliers y/o restaurar hold), a confirmar en banco. **Falta (hardware):** ver el patrón de dropout del ADC v3 real y afinar el guard. [@muestreador + @firmware]
